...
Status: one private version is running; local launch identity reconciliation is
in progress. There has been no second push.

## Purpose and fixed artifact

Execute the complete frozen R3 notebook on Kaggle with internet disabled, then
retrieve its exact committed version and independently validate the discovered
input coverage, CSV, lineage, and manifests. This exercises DeepCenter and the
...
events. These observations must be refreshed before dispatch.
...
Use a documented admission upper bound of 1.10 quota hours per runtime hour.
The extra 10% is local planning policy for accounting uncertainty, not a Kaggle
claim. Protect two 12-hour final attempts at this bound: 26.40 quota hours.
A 9,000-second (2.5-hour) platform timeout reserves another 2.75 quota hours.
At the observed balance, 0.67 hours remain outside both reservations. A timeout
...
Launch through the API without an interactive GPU session. Persist the run,
decision, exact package binding, authorization source, quota evidence and launch
intent before the single push. An ambiguous response forbids retry until
reconciled. Capture quota afterward and reconcile the actual debit; do not
report the documented bound as measured or silently reuse it after an overrun.
...
## Live launch and provider URL canonicalization
...
The authenticated notebook UI shows a private T4x2 run. The SDK request using
`GetKernel(version_label="v1")` retrieves its source; all 14 cell sources match
...
hash remain preserved, and the 2.75-hour reservation remains outstanding.
...
one Windows symlink-privilege skip. This verifies local behavior, not execution
completion. The live log records four merged prediction graphs at 906.9 seconds;
later notebook postprocessing is still running.

Completion, output validation and quota reconciliation are still required.
Local evidence is under
`reports/campaigns/e0-r3-kaggle-full-20260908-01/`.

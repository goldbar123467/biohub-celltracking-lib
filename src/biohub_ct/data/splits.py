...
def make_dataset_splits(data_dir: Path | str, *, k: int = 5) -> dict[str, dict[str, list[str]]]:
    ...
    fold_count = max(1, min(k, len(group_names)))
    ...
    for fold_index in range(fold_count):
        ...
        splits[f"fold{fold_index + 1}"] = {"train": sorted(train), "val": sorted(val)}
    return splits


def write_splits(data_dir: Path | str, output_path: Path | str, *, k: int = 5) -> dict[str, dict[str, list[str]]]:
